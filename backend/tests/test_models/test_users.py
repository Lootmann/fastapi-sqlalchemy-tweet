...
def test_user_model_repr():
    ...
    assert str(user) == f"<User (id, name, tweets, likes) = ({user.id}, {user.name}, [], [])>"
...
def test_user_model_repr_with_tweet():
    ...
    assert str(user) == f"<User (id, name, tweets, likes) = (1, hoge, [{tweet_str}], [])>"
...
def test_user_model_rep_with_tweet_and_like():
    ...
    assert str(user) == f"<User (id, name, tweets, likes) = (1, hoge, [{tweet_str}], [{like_str}])>"
